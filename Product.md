...
* 没有vhost，需要自己做开发支持。
* 跑几天后连上没有数据。抓包显示到NetStream.Play.Start和|RtmpSampleAccess就没有数据了。
* flash播放nginx-rtmp流，随机的会不能播放，抓包显示有数据。编码器推一会之后譬如100秒，flash的NetStream.time若从100开始就没有问题，若从0开始就会卡死。
...
其实srs是我到新公司后，业余时间做的一个开源服务器，初衷是公司编码器需要输出到rtmp服务器，nginx-rtmp又经常出问题，我以前也做过rtmp边缘服务器，所以干脆就自己做个rtmp服务器。主要参考nginx-rtmp的协议栈，fms的一些概念，适配当前国内主流的视频分发需求。
