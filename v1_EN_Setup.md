[**HOME**](Home) > [**EN(1.0)**](v1_EN_Home) > [**Setup**](v1_EN_Setup)
...
Use RTMP or HLS? Or HDS? Read [RTMP](v1_EN_DeliveryRTMP)、[HLS](v1_EN_DeliveryHLS)、[RTMP PK HLS](v1_EN_RTMP.PK.HTTP)

How to build RTMP cluster? Why use edge, not the forward? How to enable ATC for fault backup? Read [Forward](v1_EN_Forward)、[Edge](v1_EN_Edge)、[ATC](v1_EN_RTMP-ATC)
...
How to DVR? What is the segment plan and session plan? Read [DVR](v1_EN_DVR)
