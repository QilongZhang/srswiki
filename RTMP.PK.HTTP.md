...
互联网上的两种主要的分发方式：[HLS](https://github.com/winlinvip/simple-rtmp-server/wiki/DeliveryHLS)和[RTMP](https://github.com/winlinvip/simple-rtmp-server/wiki/DeliveryRTMP)，什么时候用谁，完全决定于应用场景。
...
另外，HTTP的也分为几种：
* HTTP progressive：早期流媒体服务器分发http文件时，以普通的http文件分发，这种叫做渐进式下载，意思就是如果文件很大譬如1小时市场1GB大小，想从中间开始播放是不行的。但这种方式已经是作古了，很多http服务器支持http文件的seek，就是从中间开始播放。
...
参考[HTTP](https://github.com/winlinvip/simple-rtmp-server/wiki/DeliveryHLS)和[RTMP](https://github.com/winlinvip/simple-rtmp-server/wiki/DeliveryRTMP)
...
* PC+点播：使用HTTP或者HLS。
* IOS：都使用HLS（实时性要求高得自己解析RTMP，不知是否可行）
