...
1. [分发RTMP流集群](v2_CN_SampleRTMPCluster)
...
1. [转发流到其他服务器](v2_CN_SampleForward)
...
如何搭建RTMP集群？如何支持全国各地的观众观看？如何跨网分发流？如何预推流到边缘？CDN的RTMP集群如何使用SRS搭建？如何构建自己的RTMP直播分发CDN？Forward和Edge的区别是什么？为何优先使用Edge而不是Forward？什么是HSTRS？SRS是否支持HTTP-FLV的合并回源？如何搭建HTTP-FLV集群？如何支持绝对时间？如何做主备？请读[Forward](v2_CN_Forward)、[Edge](v2_CN_Edge)、[HTTP Stream](v2_CN_DeliveryHttpStream)、[ATC](v2_CN_RTMP-ATC)
...
如何做认证？如何支持Referer防盗链？如何支持RTMP token穿越？请读[Callback](v2_CN_HTTPCallback)、[DRM](v2_CN_DRM)
