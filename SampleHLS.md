...
```bash
./configure --with-hls --with-nginx && make
```
...
备注：hls_path必须存在，srs只会自动创建${hls_path}下的app的目录。参考：[HLS: HLS流程](https://github.com/winlinvip/simple-rtmp-server/wiki/DeliveryHLS#hls%E6%B5%81%E7%A8%8B)
