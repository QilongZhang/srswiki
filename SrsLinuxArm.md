...
* arm推送RTMP到SRS：从arm上将h.264裸码流包装成flv流，使用[srs-librtmp](https://github.com/winlinvip/simple-rtmp-server/wiki/v1_SrsLibrtmp)，或者librtmp，将flv格式的包以RTMP发送到SRS。
...
* --with-ssl：默认开启，可关闭。支持复杂握手。参考：[握手协议](https://github.com/winlinvip/simple-rtmp-server/wiki/v1_RTMPHandshake)
* --with-hls：默认开启，可关闭。支持将RTMP流切成HLS片。注意不会编译nginx，在i386/x86_64平台上srs会编译nginx用于分发。
* --with-http-server: 默认开启，可关闭。开启[内嵌的http服务器分发hls](https://github.com/winlinvip/simple-rtmp-server/wiki/v1_HTTPServer)。
* --with-http-api: 默认开启，可关闭。支持[http api](https://github.com/winlinvip/simple-rtmp-server/wiki/v1_HTTPApi)。
