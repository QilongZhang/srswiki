...
* --with-hls 支持HLS输出，将RTMP流切片成ts，可用于支持移动端HLS（IOS/Android），不过PC端jwplayer也支持HLS。参考：[HLS](https://github.com/simple-rtmp-server/srs/wiki/v1_CN_DeliveryHLS)
* --with-dvr 支持将RTMP流录制成FLV。参考：[DVR](https://github.com/simple-rtmp-server/srs/wiki/v1_CN_DVR)
* --with-nginx 编译nginx，使用nginx作为web服务器分发HLS文件，以及demo的静态页等。
* --with-http-callback 支持http回调接口，用于认证，统计，事件处理等。参考：[HTTP callback](https://github.com/simple-rtmp-server/srs/wiki/v1_CN_HTTPCallback)
* --with-http-api 打开HTTP管理接口。参考：[HTTP API](https://github.com/simple-rtmp-server/srs/wiki/v1_CN_HTTPApi)
* --with-http-server 打开内置HTTP服务器，支持分发HTTP流。参考：[HTTP Server](https://github.com/simple-rtmp-server/srs/wiki/v1_CN_HTTPServer)
...
* --with-librtmp 客户端推流/播放库，参考[srs-librtmp](https://github.com/simple-rtmp-server/srs/wiki/v1_CN_SrsLibrtmp)
* --with-arm-ubuntu12 交叉编译ARM上运行的SRS，要求系统是Ubuntu12。参考[srs-arm](https://github.com/simple-rtmp-server/srs/wiki/v1_CN_SrsLinuxArm)
* --jobs[=N] 开启的编译进程数，和make的-j（--jobs）一样，在configure时可能会编译nginx/ffmpeg等工具，可以开启多个jobs编译，可以显著加速。参考：[Build: jobs](https://github.com/simple-rtmp-server/srs/wiki/v1_CN_Build#wiki-jobs%E5%8A%A0%E9%80%9F%E7%BC%96%E8%AF%91)
