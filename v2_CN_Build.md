...
* --with-hls 支持HLS输出，将RTMP流切片成ts，可用于支持移动端HLS（IOS/Android），不过PC端jwplayer也支持HLS。参考：[HLS](https://github.com/winlinvip/simple-rtmp-server/wiki/v2_CN_DeliveryHLS)
* --with-hds 支持HDS输出，OSMF可以播放HDS流。参考：[HDS](https://github.com/winlinvip/simple-rtmp-server/wiki/v2_CN_DeliveryHDS)
