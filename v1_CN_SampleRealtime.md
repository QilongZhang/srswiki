...
配置最低延迟的服务器详细信息可以参考：[LowLatency][LowLatency]，本文举例说明部署的实例步骤。
...
<strong>第三步，编写SRS配置文件。</strong>详细参考[LowLatency][LowLatency]
...
<strong>第四步，启动SRS。</strong>详细参考[LowLatency][LowLatency]

```bash
./objs/srs -c conf/realtime.conf
```

<strong>第五步，启动推流编码器。</strong>详细参考[LowLatency][LowLatency]
...
<strong>第六步，观看RTMP流。</strong>详细参考[LowLatency][LowLatency]
